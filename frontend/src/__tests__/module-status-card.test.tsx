...
// Every module in the sidebar is live from Step 9 - Admin, Settings and Notifications were the
...
describe("ModuleStatusCard", () => {
  it("renders the module's own copy and the step it arrives in", () => {
    ...
    expect(textOf(container)).toMatch(/Arrives in\s*Step 10/);
  });
  ...
  it("shows no number other than the arrival step", () => {
    const { container } = render(<ModuleStatusCard item={planned} />);
  ...
  it("has no live module left to render, which is the point of Step 9", () => {
