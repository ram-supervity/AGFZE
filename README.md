...
![License](https://img.shields.io/badge/License-Not_specified_%E2%80%94_see_%C2%A7_License-8b8b8b)
![Last commit](https://img.shields.io/badge/Last_commit-2026--08--29-3e3e3e)

<br/>

**Screenshots:** none in the repo yet — run `make setup && make seed-demo` and the stack shows you everything it can do.
...
- ✅ Mailbox intake (webhook + delta poll, dedup), AI classification & schema-driven extraction with confidence gating
- ✅ Deterministic batch matching, transaction lifecycle (received → … → committed), FA and sales legs
- ✅ Rule engine (BR-02…BR-07, BR-13, IV-01, SL-01, LG-01 evaluated; all fourteen defined), exceptions, approvals
- ✅ Shipments, containers, OBLs, issues, staleness sweep
- ✅ SAP / DMS / tracker preparation with honest manual paths; retry sweep with real backoff
- ✅ Daily + monthly reports with drill-through figures and template configuration
- ✅ Notifications: in-app, email, Web Push
- ✅ PWA with read-only offline; command palette; onboarding walkthrough
- ✅ GCP Terraform, CI/CD, rollback, verify-production, restore-test

**Planned / open, as signalled by the code itself**

- 🔜 **BR-01, BR-08…BR-12** — registered with `implemented=False`, awaiting their data or their decision; BR-08's failure-routing and BR-11's provenance retention are the highest-value next evaluators
- 🔜 **Carrier tracking adapters** — the registry is deliberately empty; implementing `fetch` and calling `register_adapter` is the whole of the integration work, nothing above it changes
- 🔜 **Real SAP BAPI/OData and DMS contracts** — currently unconfigured by design; a confirmed endpoint turns the honest manual path into a real posting with no change to the outcome protocol
- 🔜 **Tracker workbook confirmation** — AGFZE must name drive, workbook, table and columns; the client, row-level and complete, is waiting
- 🔜 **Entra ID broker activation** — the realm ships the broker disabled with `REPLACE-ME…` placeholders and an exact fill-in list in `infra/keycloak/README.md`
- 🔜 **Document retention** — the mechanism is built and switched off; the period is AGFZE's to confirm, and anything else would be irreversible
- 🔜 **Transaction `CLOSED`** — declared and deliberately unreachable until payment confirmation, documentation and shipment completeness are specified
- 🔜 **Azure Blob vs GCS** — the PRD names Azure Blob; the storage backend is written against GCS because that is the estate that exists. The discrepancy is explicitly recorded, not silently resolved
- 🔜 **Counterparty master data** — codes are derived on read (never stale); a stable identifier surviving a name correction needs the master-data table nobody has yet asked for
- 🔜 **Screenshots** for this README — run the stack, `make seed-demo`, and send a PR

---

## ⚖️ License

**No license file is present in this repository**, and the GitHub API reports no license. That means the default copyright status applies: all rights reserved by the author until a license is added. If this is your project and you want others to build on it, adding an `LICENSE` file (MIT, Apache-2.0, or whatever your legal team prefers) is a two-minute change that turns "see the code" into "use the code".
